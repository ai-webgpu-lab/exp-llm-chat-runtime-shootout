...
`런타임별 로컬 채팅 비교`를 단일 질문으로 분리해 검증하는 experiment 저장소입니다. 하나의 가설과 baseline에 집중하고, 결과를 재현 가능한 형식으로 남겨 이후 benchmark/app 저장소의 입력으로 사용합니다.
...
## 핵심 질문
- 런타임별 로컬 채팅 비교를 브라우저/WebGPU 환경에서 재현 가능한 baseline으로 만들 수 있는가
...
- 이 저장소의 live demo는 `https://ai-webgpu-lab.github.io/exp-llm-chat-runtime-shootout/`입니다.
